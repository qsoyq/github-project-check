...
说明：模板中的 `enforce_admins: false` 表示管理员可绕过分支保护合并，适用于个人仓库或没有第二个 reviewer 的 bootstrap 阶段；PR 作者仍不能 approve 自己的 PR。团队仓库或已有多个维护者时，应改为 `enforce_admins: true`，让管理员也受保护规则约束。API 成功返回只能证明本次请求已被平台接受；后续仍建议用 `gh api repos/<owner>/<repo>/branches/<branch>/protection` 或 GitHub UI 复核实际状态。
